...
def init(id, pwd, ua, mobile_device, headless, newsave):
    ...
    # 드라이버 생성 (ChromeDriverManager 사용)
    ...
    # ID 입력 후 Tab으로 PW 필드로 이동, PW 입력 후 Enter로 로그인
    try:
        username.clear()
        username.send_keys(input_id)
        time.sleep(0.8)
        ...
        logging.warning("키 입력 방식으로 로그인 입력 중 오류 발생: %s", e)
        # 예외가 날 경우 기존 방식으로 시도해본다 (fallback)
        try:
...
def add_options_mobile_device(id, pwd, ua, mobile_device, headless, driver):
    ...
    # 새로운 드라이버 생성 (ChromeDriverManager 사용)
    new_driver = webdriver.Chrome(service=Service(ChromeDriverManager().install()), options=new_options)
    ...
    for cookie in driver.get_cookies():
        try:
            ...
            # 일부 쿠키는 도메인/보안 설정으로 추가되지 않을 수 있음
            pass
